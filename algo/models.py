...
class Actor_eval(nn.Module):
    ...
    def forward(self, x, g=None):
        if g is not None:
            ...
            x = F.relu(self.l1(torch.cat([x.float(), g.float()], 1)))
        else:
            x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        return x 
...
class ControllerActor(nn.Module):
    def __init__(self, state_dim, goal_dim, action_dim, scale=1, eval=False):
        ...
        self.scale = nn.Parameter(torch.tensor(scale).float(),
                                  requires_grad=False)
        if eval:
            self.actor = Actor_eval(state_dim, goal_dim, state_dim)
        else:
