...
class FKMInterface:
    ...
    def __call__(self, obs, actions, batch_size=256, deterministic=False, rand_c=False, goal=None, goal_dim=None):
        goal_dim=2
        goal=torch.Tensor(np.array([0, 8.]))
        ...
        if rand_c:
            ...
            for i in range(num_models):
                score = -torch.norm(goal - ensemble_samples[model_inds, batch_inds][:, :goal_dim])
                if score > max_score:
                    max_score = score
                    model_inds = i
