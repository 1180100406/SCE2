...
import gym
import copy
import algo.explors.models as models
import algo.explors.utils
...
class EnvTeacher(gym.Env):
    ...
    def __init__(self, env, args, teacher_name):
        ...
        self.n_steps_to_follow_orig = 1e5 #2000
    ...
    def get_reward(self, state, action, next_state):
        ...
        if self.teacher_name == "Orig":
            ...
            return self.Rexplore(next_state)

        elif self.teacher_name == "SelfRS":
            return self.Rexploit(state, action) if self.switch_teacher \
                else 0
            ...
            return self.Rexploit(state, action) + self. Rexplore(next_state) if self.switch_teacher \
                else self.Rexplore(next_state)
            ...
            return self.R_lirpg(state, action) if self.switch_teacher \
                else 0
    ...
    def R_sors(self, state, action):
        return self.rsors_network.network(torch.cat([torch.Tensor(state), torch.Tensor(action)], dim=0)) * self.clipping_epsilon
    #enddef
    ...
    def update_sors(self, D):
        ...
        for i, j, _ in pairwise_data:
            ...
            for t, (s_i, a_i, _, _, _, _, _, _) in enumerate(episode_i):
                return_r_phi_i += (self.gamma ** t) * self.rsors_network.network(torch.cat([torch.Tensor(s_i), torch.Tensor(a_i)], dim=0)).detach().numpy()
                
                gradient_s_a_i = (self.gamma ** t) * torch.sum(self.rsors_network.network(torch.cat([torch.Tensor(s_i), torch.Tensor(a_i)], dim=0)))
            ...
            for t, (s_j, a_j, _, _, _, _, _, _) in enumerate(episode_j):
                return_r_phi_j += (self.gamma ** t) * self.rsors_network.network(torch.cat([torch.Tensor(s_j), torch.Tensor(a_j)], dim=0)).detach().numpy()
                
                gradient_s_a_j = (self.gamma ** t) * torch.sum(self.rsors_network.network(torch.cat([torch.Tensor(s_j), torch.Tensor(a_j)], dim=0)))
    ...
    def postprocess_data(self, D):
        ...
        for i in range(num):
            ...
            postprocessed_epidata, j = self.get_postposessed_episode(self.env, D.storage, tmp, num)
            tmp = j
    ...
    def get_postposessed_episode(self, env_orig, episode, num, count):
        ...
        for i in range(len(postprocessed_epidata) - 1, -1, -1):  # iterate backwards
            ...
            if count > 1 and i == k-1 and k < 500:
                r_bar = 1
            G_bar = r_bar + 0.99 * G_bar
            postprocessed_epidata[i][7] = G_bar  # update G_bar in episode

        return postprocessed_epidata, num+k
    #enddef
