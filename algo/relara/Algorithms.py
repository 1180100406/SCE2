...
class ReLaraAlgo:
    ...
    def __init__(self, env, pa_actor_class, pa_critic_class, ra_actor_class, ra_critic_class, exp_name="ReLara",
                 ...
                 save_frequency=100000, save_folder="./ReLara/", env_name=None):
        """
        ...
        if 'Ant' in env_name or 'Pusher' in env_name or 'Reacher' in env_name:
            observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=env.observation_space['observation'].shape, dtype=np.float32)
            action_space = gym.spaces.Box(low=env.action_space.low[0], high=env.action_space.high[0], shape=env.action_space.shape, dtype=np.float32)
        else:
            observation_space = env.observation_space
            action_space = env.action_space

        self.ra_obs_space = gym.spaces.Box(low=-1, high=1,
                                           shape=(observation_space.shape[0] + action_space.shape[0],),
        ...
        run_name = "{}-{}-{}-{}".format(exp_name, 1, seed,
        ...
        self.pa_actor = pa_actor_class(observation_space, action_space).to(self.device)
        self.pa_qf_1 = pa_critic_class(observation_space, action_space).to(self.device)
        self.pa_qf_2 = pa_critic_class(observation_space, action_space).to(self.device)
        self.pa_qf_1_target = pa_critic_class(observation_space, action_space).to(self.device)
        self.pa_qf_2_target = pa_critic_class(observation_space, action_space).to(self.device)
