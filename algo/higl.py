...
class Controller(object):
    def __init__(self,
        ...
        self.eval = eval
        self.actor = ControllerActor(state_dim, goal_dim, action_dim, scale=max_action, eval=eval)
        self.actor_target = ControllerActor(state_dim, goal_dim, action_dim, scale=max_action, eval=eval)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=actor_lr)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, fkm_obj=None, mgp_lambda=.0, \
              osrp_lambda=.0, manage_replay_buffer=None, manage_actor=None, manage_critic=None, sg_scale=None, eval=False, start_con=0):
        ...
        for it in range(iterations):
            ...
            if eval == True:
                ...
                act_loss = torch.pow(torch.norm(next_state - self.select_action(state, sg, to_numpy=False), dim=1), 2).sum(dim=0) / batch_size
                # Optimize the actor
                self.actor_optimizer.zero_grad()
                act_loss.backward()
                self.actor_optimizer.step()
    ...
    def save(self, dir, env_name, algo, version, seed):
        if self.eval is True:
            torch.save(self.actor.state_dict(), "{}/{}_{}_{}_{}_CheckConActor.pth".format(dir, env_name, algo, version, seed))
            torch.save(self.critic.state_dict(), "{}/{}_{}_{}_{}_CheckConCritic.pth".format(dir, env_name, algo, version, seed))
            torch.save(self.actor_target.state_dict(), "{}/{}_{}_{}_{}_CheckConActorTarget.pth".format(dir, env_name, algo, version, seed))
            torch.save(self.critic_target.state_dict(), "{}/{}_{}_{}_{}_CheckConCriticTarget.pth".format(dir, env_name, algo, version, seed))
        else:
