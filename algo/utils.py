...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size, new_first=False, start=0):
        if self.start < len(self.storage[0]):
            self.last_start = self.start
        if new_first:
            if len(self.storage[0])-batch_size >= 0:
                ind = np.arange(len(self.storage[0])-batch_size, len(self.storage[0]))
            else:
                ind = np.arange(0, len(self.storage[0]))
                rand_ind = np.random.randint(0, len(self.storage[0]), size=batch_size-len(self.storage[0]))
                ind = np.concatenate([ind, rand_ind], axis=0)
        else:
            if start < len(self.storage[0]):
                ind = np.random.randint(start, len(self.storage[0]), size=batch_size)
            else:
...
def train_re_adj_net(re_a_net, a_net, states, optimizer, goal_dim, margin_pos, margin_neg,
                  n_epochs=100, batch_size=64, device='cpu', verbose=False):
    if verbose:
        print('Generating training data...')
    dataset = ReANetDataset(a_net(torch.Tensor(np.array(states)[:, :goal_dim])), states)
    if verbose:
        print('Totally {} training pairs.'.format(len(dataset)))
    dataloader = Data.DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False, generator=torch.Generator(device))
    n_batches = len(dataloader)
    ...
    for i in range(n_epochs):
        ...
        for j, data in enumerate(dataloader):
            ...
            x = re_a_net(x)
            loss = torch.pow(torch.norm(x - label, dim = 1), 2).sum(dim=0) / label.shape[0]
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if verbose and (j % 50 == 0 or j == n_batches - 1):
...
class ReANetDataset(Data.Dataset):

    def __init__(self, a_net_output, states):
        super().__init__()
        n_samples = np.array(states).shape[0]
        self.x = []
        self.label = []
        for i in range(n_samples - 1):
            self.x.append(a_net_output[i].detach().cpu().numpy())
            self.label.append(states[i])
        self.x = np.array(self.x)
        self.label = np.array(self.label)
