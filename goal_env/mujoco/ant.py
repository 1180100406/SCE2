...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, file_path=None, expose_all_qpos=True,
                 expose_body_coms=None, expose_body_comvels=None, fix_starting_point=False, init_by_seed=False,
                 stochastic_xy=False, stochastic_sigma=0.05):
        self._expose_all_qpos = expose_all_qpos
        self._expose_body_coms = expose_body_coms
        self._expose_body_comvels = expose_body_comvels
        self._body_com_indices = {}
        self._body_comvel_indices = {}
        self.fix_starting_point = fix_starting_point
        self.init_by_seed = init_by_seed
    ...
    def step(self, a):
        ...
        # stochasticity
        if self.stochastic_xy:
            qpos = np.copy(self.data.qpos)
            qvel = np.copy(self.data.qvel)
            qpos[:2] += np.random.normal(scale=self.stochastic_sigma, size=2)
    ...
    def reset_model(self):
        if self.fix_starting_point:
            qpos = self.init_qpos
            qvel = self.init_qvel
        elif self.init_by_seed:
